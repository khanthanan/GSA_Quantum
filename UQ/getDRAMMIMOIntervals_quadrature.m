...
function [credLims, predLims] = getDRAMMIMOIntervals_quadrature(data, model, modelParams, chain_q, chain_cov_err, nSample)
    disp('--------------------------------------------------');
    ...
    NN = 8; %HARDCODED FOR THIS MODEL!!!
    for iisample = 1 : 1 : nSample
        qi = chain_q(isample(iisample), :)';
        randError = mvnrnd(zeros(NN, 1), chain_cov_err(:, :, isample(iisample)));
        for i = 1 : 1 : N
